...
export default function App() {
  ...
  return (
    <div className="flex h-screen bg-[#F8FAFC] overflow-hidden w-full font-sans text-gray-800">
      ...
      {!isSidebarOpen && (
        <button 
          className="md:hidden fixed top-4 left-4 z-50 p-2 bg-white rounded-lg shadow-sm border border-gray-100 text-[#003B73]"
      ...
      {/* Sidebar */}
      <aside 
        className={`fixed md:static inset-y-0 left-0 z-40 w-64 bg-[#003B73] text-white flex flex-col transition-transform duration-300 ease-in-out ${
      ...
      >
        ...
        <div className="p-4 border-t border-white/10">
          ...
          >
            ...
            <span className={`transition-opacity ${!isSidebarOpen ? 'md:hidden lg:inline' : ''}`}>
        ...
        <header className="h-16 bg-white border-b border-gray-200 flex items-center justify-between px-4 sm:px-6 shrink-0 shadow-sm z-10">
          <div className="flex items-center space-x-4">
             <div className="hidden md:block">
               <h2 className="text-lg font-bold text-[#003B73] capitalize">
                  {activeTab === 'dashboard' ? 'Tổng quan Doanh nghiệp' : 
                   activeTab === 'ai' ? 'Trợ lý Quyết định AI' : 
                   activeTab === 'workforce' ? 'Workforce Decision Engine' : 
                   activeTab === 'whatif' ? 'What-if Scenario Simulator' : 
                   'Decision Timeline'}
               </h2>
             </div>
          </div>
          <div className="flex items-center space-x-3">
             <div className="text-right hidden sm:block">
               <p className="text-sm font-bold text-gray-800">Ông Nguyễn Văn A</p>
               <p className="text-xs text-gray-500">Chủ tịch HĐQT</p>
             </div>
