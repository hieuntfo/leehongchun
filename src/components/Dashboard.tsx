  ...
  return (
    ...
    >
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="col-span-1 md:col-span-3 bg-gradient-to-r from-[#003B73] to-[#0077B6] rounded-2xl p-6 text-white shadow-lg relative overflow-hidden">
          ...
          <div className="relative z-10 flex flex-col md:flex-row md:items-end justify-between">
            <div>
              <p className="text-blue-100 text-sm font-medium mb-2 uppercase tracking-wider">Tốc độ ra quyết định (Decision Speed Time)*</p>
              <div className="flex items-end space-x-3 mb-1">
                <span className="text-6xl font-black leading-none text-[#FFC857] drop-shadow-md">7.6</span>
                <span className="text-2xl pb-1 font-semibold">giờ</span>
              </div>
              <div className="mt-4 flex items-center space-x-4">
                <div className="bg-white/20 px-3 py-1.5 rounded-lg border border-white/30 backdrop-blur-sm text-sm font-medium flex items-center">
                  <span className="opacity-80 mr-2">Hiện trạng cũ:</span> <span className="line-through opacity-60">14 ngày</span>
                </div>
                <div className="text-sm font-bold bg-green-500 text-white px-3 py-1.5 rounded-lg shadow inline-flex items-center">
                  <TrendingDown size={16} className="mr-1" /> Cải thiện 97.7%
                </div>
              </div>
            </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="bg-white p-5 rounded-xl shadow-sm border border-gray-100 flex flex-col justify-between">
          ...
          <div>
            <p className="text-sm text-gray-500 font-medium">Doanh thu hợp nhất (YTD)</p>
            <p className="text-2xl font-bold text-gray-900 leading-tight mt-1">425 tỷ ₫</p>
            <p className="text-xs text-green-600 font-medium mt-2 flex items-center"><TrendingUp size={12} className="mr-1"/> +12% YoY* so với ngân sách</p>
          </div>
        </div>
        ...
        <div className="bg-white p-5 rounded-xl shadow-sm border border-gray-100 flex flex-col justify-between">
          ...
          <div>
            <p className="text-sm text-gray-500 font-medium">Tỷ suất EBITDA*</p>
            <p className="text-2xl font-bold text-gray-900 leading-tight mt-1">20.8%</p>
            <p className="text-xs text-green-600 font-medium mt-2 flex items-center"><TrendingUp size={12} className="mr-1"/> +4.3pp* YoY*</p>
          </div>
        </div>

        <div className="bg-white p-5 rounded-xl shadow-sm border border-orange-100 bg-orange-50/10 flex flex-col justify-between">
          <div className="flex items-start justify-between mb-4">
        ...
        <div className="bg-white p-5 rounded-xl shadow-sm border border-red-100 bg-red-50/10 flex flex-col justify-between">
          <div className="flex items-start justify-between mb-4">
          ...
          <div>
            <p className="text-sm text-gray-600 font-medium">Nợ phải thu quá hạn &gt;60 ngày</p>
            <p className="text-2xl font-bold text-gray-900 leading-tight mt-1">23 tỷ ₫</p>
            <p className="text-xs text-red-600 font-medium mt-2">Cảnh báo: 4 khách hàng trọng điểm</p>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="h-[250px] w-full">
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={data} margin={{ top: 5, right: 30, left: 10, bottom: 5 }}>
                ...
                <YAxis axisLine={false} tickLine={false} tick={{ fontSize: 12, fill: '#6B7280' }} tickFormatter={(val) => `${val}T`} />
                <Tooltip 
                  contentStyle={{ borderRadius: '8px', fontSize: '13px', border: '1px solid #e2e8f0', boxShadow: '0 10px 15px -3px rgb(0 0 0 / 0.1)' }}
                  formatter={(value: number) => [`${value} tỷ ₫`, 'Doanh thu']}
                />
                <Line type="monotone" dataKey="rev" stroke="#0077B6" strokeWidth={4} dot={{ strokeWidth: 2, r: 5, fill: 'white' }} activeDot={{ r: 8 }} />
              </LineChart>
            </ResponsiveContainer>
          </div>
        </div>
        ...
        <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100 flex flex-col">
          <h3 className="text-base font-semibold text-gray-800 mb-4 flex items-center border-b border-gray-100 pb-3">
            ...
            Alert Engine (Cảnh báo thông minh)
          </h3>
          <div className="space-y-3 flex-1 overflow-auto">
            {[
              ...
              <div key={i} className={`p-4 rounded-lg border-l-4 shadow-sm text-sm bg-white border ${
                ...
                <span className="font-bold text-gray-900 block mb-1">Phân hệ {alert.id}</span>
                <span className="text-gray-600 block">{alert.text}</span>
              </div>
            ))}
          </div>
        </div>
